...
export type Profile = { userId: string; displayName?: string; email?: string; primarySkill?: string; location?: string; cred?: number; onboardingComplete?: boolean; identityStatus?: string; trustStatus?: 'UNVOUCHED' | 'PENDING_VOUCH' | 'VOUCHED'; passportNo?: string; posterVerificationStatus?: string; verificationStatus?: string; tier?: string; trustScore?: number; vouchCount?: number };
...
export function createApi(accessToken: string) {
  const request = async <T>(path: string, init: RequestInit = {}): Promise<T> => {
    ...
    if (!response.ok) { const message = data.error === 'ACTIVE_PASSPORT_REQUIRED' ? 'Get vouched before posting or applying for gigs.' : data.error || data.message || `Request failed (${response.status})`; throw new Error(message); }
    return data as T;
  };
  return {
