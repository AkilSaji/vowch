...
test('super admins retain full admin access through legacy and Cognito group claims', () => {
  ...
  assert.equal(hasAdminPermission(eventFor({ 'cognito:groups': '["vowch-super-admin"]' }), 'identity:read'), true);
  assert.equal(hasAdminPermission(eventFor({ 'cognito:groups': ['vowch-super-admin'] }), 'admin:manage'), true);
});
...
test('admin routes resolve to their least-privilege permission', () => {
  ...
  assert.equal(adminPermissionFor('/v1/admin/access', 'POST'), 'admin:manage');
